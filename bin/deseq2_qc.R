...
# 1. Calculate the median for each gene (row) across all samples
gene_medians <- apply(assay(dds, vst_name), 1, median)

# 2. Subtract the median from each gene's expression value
# Note: R subtracts vectors column-wise by default, so we transpose, 
# subtract, and transpose back to maintain the original shape.
rle_matrix <- t(apply(assay(dds, vst_name), 1, function(x) x - median(x)))

# 3. Convert to "Long" format for ggplot using Base R
# We convert the matrix to a data frame and use stack()
rle_df <- data.frame(
  RLE_Value = as.vector(rle_matrix),
  Sample = rep(colnames(rle_matrix), each = nrow(rle_matrix)),
  Gene = rep(rownames(rle_matrix), times = ncol(rle_matrix))
)

# 4. Plotting (The ggplot2 code remains the same as it is not part of the restricted packages)
ggplot(rle_df, aes(x = Sample, y = RLE_Value, fill = Sample)) +
  ...
  labs(title = "Relative Log Expression (RLE)",
       subtitle = "Centers should be at 0; deviations indicate sample bias",
       y = "Relative Log Expression",
       x = "")

write.table(rle_matrix, file = paste(opt$outprefix, ".rle.vals.txt", sep=""),
